...
namespace PersistentLocos
{
    ...
    public static class StartGameData_NewCareerPatch
    {
        static void Postfix()
        {
			if (!PL_Multiplayer.CanModifyWorld)
			{
				Main.Log("[MP] Client skipped new-career initialization.");
				return;
			}

			LocoSpawnState.Reset();
			PL_Multiplayer.NotifyLocoCountChanged();
	...
	internal static class Patch_LocoZoneBlocker_DisableBrakeOverride
	{
		static bool Prefix()
		{			
			return PL_Multiplayer.IsClient;
		}
	}
    ...
    class Patch_SaveGameManager_Start
    {
        ...
        {			
			if (!PL_Multiplayer.CanUseSaveData)
			{
				Main.Log(
					"[MP] Client save-data writer disabled.");
    ...
    class Patch_StartGameData_FromSaveGame_MakeCurrent
    {
        static void Postfix(StartGameData_FromSaveGame __instance)
        {
			...
			{
				Main.Log(
					"[MP] Client ignored local PersistentLocos save data.");
    ...
    public static class LocoSpawnState
	{
		...
		private const string CountSaveKey =
			"PersistentLocos_LocoCount";

		private const string LimitSaveKey =
			"PersistentLocos_LocoLimit";
			...
			SaveGameData saveData)
		{
			...
			if (maybeCount.HasValue)
			{
				...
				Main.Log(
					$"Loaded locomotive count from save: " +
					$"{_count}");
			}
			else
			{
			...
			if (maybeLimit.HasValue)
			{
				...
				Main.Log(
					$"Loaded LocoLimit from save: " +
					$"{Main.Settings.LocoLimit}");
			}
			else
			{
				Main.Log(
					"No LocoLimit saved – using settings value.");
			}
		...
		public static void Decrement()
		{
			...
			{
				Main.Log(
					"LocoSpawnState.Decrement called but " +
					"count is already 0 – ignoring.");
			}
		}

		public static void SetFromHost(
			int count)
		{
			_count =
				Math.Max(
...
namespace PersistentLocos.Plus
{
	...
	internal static class LocoDebtController_StageOnDestroy_BlockWhenPersistentDamage
	{
		...
		{
			if (!PersistentLocos.PL_Multiplayer
				.CanModifyWorld)
			{
				return true;
			}
	...
	internal static class LocoDebtController_LoadDestroyedDebts_ClearWhenPersistentDamage
	{
		...
		{
			if (!PersistentLocos.PL_Multiplayer.CanModifyWorld)
			{
				return;
			}
			if (!PersistentLocos.Main.Settings.enablePersistentDamage)
	...
	internal static class CashRegister_GetTotalPrice_WithMultiplier
	{
		[HarmonyPostfix]
		static void Postfix(
			CashRegisterModule __instance,
			ref double __result)
		{
			var car =
				Helpers.ResolveCarFromCashRegister(
					__instance) as TrainCar;

			if (car == null ||
				!Helpers.IsLocomotive(car))
			{
				return;
			}

			double totalMult =
				Helpers.GetEffectiveServiceMultiplier(
					car);
